...
// Kick-threshold count: how many warnings (any type) a member already has
// logged within the trailing 6 COMPLETE months, excluding the current
// still-open one — the exact window WarningsTab.jsx's getSixMonthWindow()
// uses for its own "X / 3" kick badge. Deliberately independent of whichever
// month is being generated by the caller: kick eligibility is a live "as of
// right now" question, not scoped to the report's own period — a leader
// generating July's report in September still needs to know today's real
// standing, not July's. Counts warning ROWS, not distinct periods (matches
// WarningsTab.jsx's windowWarnings.length — two warnings landing in the same
// month both count).
export async function getKickThresholdCounts(env) {
  const now = new Date();
  const endKey = now.getUTCFullYear() * 12 + (now.getUTCMonth() + 1); // exclusive upper bound
  const startKey = endKey - 6;
  const { results } = await env.DB.prepare(`
    SELECT torn_user_id, COUNT(*) AS cnt
  ...
  const map = {};
  for (const r of (results || [])) map[r.torn_user_id] = r.cnt;
  return map;
}
  ...
  try {
    ...
    for (const r of (rows.results || [])) {
      ...
      members.push({
        ...
        // Already at/over the 3-in-6-months kick threshold from PRIOR
        // warnings alone — before whatever this report is about to add.
