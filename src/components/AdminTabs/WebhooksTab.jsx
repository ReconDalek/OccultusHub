...
const EVENT_META = {
  ...
  investment_ended: {
    ...
    description: 'Sends a Discord message the first time a bank investment\'s end date has passed, stating how much of the profit is owed back to the faction. Principal is assumed reinvested by the member, not returned. Fires once per investment — separate from the TCI purchase reminder above, which is about the interim bonus, not maturity.',
    vars: [
      ['{mention}',        'Discord @mention from the configured user ID above'],
      ['{member_mention}', 'Discord @mention of the investor (from their investment record)'],
      ['{member_name}',    'Torn username of the investor'],
      ['{end_date}',       'Investment end date (YYYY-MM-DD)'],
      ['{principal}',      'Original investment amount — assumed reinvested, NOT owed back (formatted)'],
      ['{profit}',         'Total profit earned (principal × rate, formatted)'],
      ['{member_keeps}',   'The investor\'s cut of the profit (formatted)'],
      ['{faction_income}', 'The faction\'s cut of the profit — this is what\'s actually owed (formatted)'],
      ['{faction_name}',   'Faction the investment belongs to'],
    ],
    defaultTemplate: [
      '{mention}{member_mention}',
